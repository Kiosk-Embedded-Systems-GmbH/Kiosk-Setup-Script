...
.SYNOPSIS
    ...
    VARIANT: TOUCH KEYBOARD ON - the touch keyboard is configured to appear
    automatically (docked, always-on even with an external/HID keyboard
    such as a barcode scanner attached). See Set-KioskConfig-NoKeyboard-*
    for the counterpart where the touch keyboard is disabled entirely.
...
.DESCRIPTION
    Configures in a single run:
      ...
      8. Touch keyboard: docked instead of floating, desktop auto-invoke
         on, TabletInputService enabled
...
Write-Host "=== Kiosk Setup Script v$ScriptVersion (Touch Keyboard ON) started ===" -ForegroundColor Cyan
...
$TOUCH_KEYBOARD_ACTION = {
    ...
    # Explicitly requested: enable auto-invoke in desktop mode
    New-ItemProperty -Path $tipPath -Name "EnableDesktopModeAutoInvoke" -PropertyType DWord -Value 1 -Force | Out-Null

    # IMPORTANT: EnableDesktopModeAutoInvoke alone only reproduces the
    # "Show touch keyboard when no keyboard attached" behavior - if Windows
    # detects a physical/external keyboard, it can still suppress the touch
    # keyboard despite this setting. Since Windows 11 build 22621.1926,
    # this is controlled by a separate three-way setting (Never / When no
    # keyboard attached / Always), stored in "TouchKeyboardTapInvoke"
    # (0/1/2). Setting it to 2 ("Always") forces the touch keyboard to
    # appear regardless of whether an external keyboard is connected -
    # relevant for a kiosk where a barcode scanner or other HID keyboard
    # device may otherwise make Windows think a "real" keyboard is present.
    New-ItemProperty -Path $tipPath -Name "TouchKeyboardTapInvoke" -PropertyType DWord -Value 2 -Force | Out-Null

    # Confirmed key (taken from a partner script that reliably worked in
    # practice): TipbandDesiredVisibility=1 docks the keyboard at the
    # bottom of the screen instead of the freely movable (floating)
    # Windows 11 default keyboard. "IsFloating" was removed here - the
    # partner script never sets it, and it's unconfirmed/may interfere.
    New-ItemProperty -Path $tipPath -Name "TipbandDesiredVisibility" -PropertyType DWord -Value 1 -Force | Out-Null

    # Fixes symbols missing from the touch keyboard that require the AltGr
    # ("third") layer on a physical German keyboard - e.g. @, {, }, €. The
    # default compact touch keyboard layout doesn't reliably expose these;
    # this switches to the standard/full layout, which does. Confirmed fix
    # for an equivalent missing-symbol report on a German/Swiss-German
    # touch keyboard (Microsoft Q&A).
    #
    # TEMPORARILY REMOVED for testing: suspected to conflict with
    # TipbandDesiredVisibility (docking) - the full/compatibility layout may
    # have its own display logic that ignores/overrides the docking
    # setting. Being tested in isolation on an internal device; re-add once
    # confirmed whether it's the cause of the "still floating on Windows 11"
    # issue. If re-added, uncomment below (and the HKLM counterpart in
    # Set-KioskTouchKeyboardMachineDefaults) and revert this comment.
    # New-ItemProperty -Path $tipPath -Name "EnableCompatibilityKeyboard" -PropertyType DWord -Value 1 -Force | Out-Null
    Remove-ItemProperty -Path $tipPath -Name "EnableCompatibilityKeyboard" -ErrorAction SilentlyContinue
...
# ============================================================
# Confirmed by a partner script that reliably docks the keyboard in
# practice: the SAME TabletTip keys also need to be set machine-wide under
# HKLM, not just per-user under HKCU (which is all we did before). Without
# this, Windows 11 apparently doesn't reliably honor the per-user setting.
function Set-KioskTouchKeyboardMachineDefaults {
    ...
    New-ItemProperty -Path $tipPathHKLM -Name "EnableDesktopModeAutoInvoke" -PropertyType DWord -Value 1 -Force | Out-Null
    New-ItemProperty -Path $tipPathHKLM -Name "TouchKeyboardTapInvoke" -PropertyType DWord -Value 2 -Force | Out-Null
    New-ItemProperty -Path $tipPathHKLM -Name "TipbandDesiredVisibility" -PropertyType DWord -Value 1 -Force | Out-Null
    # TEMPORARILY REMOVED - see matching comment in $TOUCH_KEYBOARD_ACTION above.
    # New-ItemProperty -Path $tipPathHKLM -Name "EnableCompatibilityKeyboard" -PropertyType DWord -Value 1 -Force | Out-Null
    Remove-ItemProperty -Path $tipPathHKLM -Name "EnableCompatibilityKeyboard" -ErrorAction SilentlyContinue
    Write-Host "  Machine-wide touch keyboard defaults set." -ForegroundColor Green
}
...
function Enable-KioskTouchKeyboardService {
    Write-Host "`n[8c/13] Enabling TabletInputService (touch keyboard service)..." -ForegroundColor Yellow

    $svc = Get-Service -Name TabletInputService -ErrorAction SilentlyContinue
    if (-not $svc) {
        # On some Windows 11 (IoT) images the service is entirely missing
        # rather than just disabled - usually because the Microsoft.Windows.
        # InputApp package didn't get registered for this device. Try to
        # re-register it; if that doesn't bring the service back either, the
        # component is genuinely absent from this image (would need an
        # in-place repair/feature install to fix, out of scope for this script).
        Write-Host "  TabletInputService not found - attempting to re-register the Windows InputApp package..." -ForegroundColor DarkYellow
        try {
            Get-AppxPackage -AllUsers Microsoft.Windows.InputApp -ErrorAction Stop | ForEach-Object {
        ...
        if (-not $svc) {
            Write-Host "  NOTE: TabletInputService is not present on this device/image. On Windows 11 (especially 24H2+)," -ForegroundColor DarkYellow
            Write-Host "  Microsoft has deprecated InputApp.exe/TabletInputService in favor of TextInputHost.exe/TabTip.exe," -ForegroundColor DarkYellow
            Write-Host "  which no longer strictly require this service. The touch keyboard likely still works despite this" -ForegroundColor DarkYellow
            Write-Host "  message - please verify on the device rather than treating this as a hard failure." -ForegroundColor DarkYellow
            return
        }
    }
    ...
    try {
        ...
        Write-Host "  TabletInputService set to Automatic and started." -ForegroundColor Green
    } catch {
...
# additional touch keyboard layouts alongside German, which stays the
# active/default one (position "1"). Selectable via the touch keyboard's
# own on-screen language switcher - relevant since Assigned Access hides
# the taskbar language bar, so the touch keyboard's built-in switcher is
# the only way "Kiosk" can actually change layout.
$KIOSK_KEYBOARD_LAYOUTS_ACTION = {
    ...
    # entries are installed alongside it, selectable via the touch
    # keyboard's own layout switcher.
...
Write-Host "  Adding additional touch keyboard layouts (FR/IT/UK/PL/CZ/ES/AR) for 'Kiosk' only..." -ForegroundColor Yellow
